...
def stamp_pdf():
    """
    Apply a translucent watermark/stamp to every page of a PDF.
    
    Expected:
        - 'file': PDF file
        - 'stamp_text': Text to use as watermark
        - 'font_size': Optional (default: 60)
        - 'opacity': Optional (default: 0.3)
    Returns: PDF with watermark applied
    """
    try:
        # Get uploaded file
        if 'file' not in request.files:
        ...
        stamp_text = request.form.get('stamp_text', '').strip()
        if not stamp_text:
            return jsonify({'error': 'Stamp text is required'}), 400
            
        try:
            font_size = float(request.form.get('font_size', 60))
        except ValueError:
            font_size = 60
        ...
        for page in doc:
            ...
            # overlay=True puts it on top, False puts it in background
            page.insert_text(
                (center_x, center_y),
                stamp_text,
                fontsize=font_size,
                rotate=0, # Changed from 45 to 0 due to PyMuPDF restriction
                color=(0.5, 0.5, 0.5), # Gray
                fill_opacity=opacity,  # Use fill_opacity for transparency
                overlay=True
            )
        
        # Save stamped PDF to bytes
        output = io.BytesIO()
        output.write(doc.tobytes())
        doc.close()
        output.seek(0)
        ...
        print(f"Error in stamp_pdf: {str(e)}")
        return jsonify({'error': f'Failed to stamp PDF: {str(e)}'}), 500
